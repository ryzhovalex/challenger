# aiosqlite - Copyright Amethyst Reese
# Licensed under the MIT license
#
# Modifications for accessibility by Alexander Ryzhov (ryzhovalex).
...
class Cursor:
    ...
    @property
    def row_factory(self) -> Optional[Callable[[sqlite3.Cursor, Record], object]]:
...
class Connection(Thread):
    ...
    def _execute_insert(self, sql: str, parameters: Any) -> Optional[Record]:
        cursor = self._conn.execute(sql, parameters)
        cursor.execute("SELECT last_insert_rowid()")
        return cursor.fetchone()

    def _execute_fetchall(self, sql: str, parameters: Any) -> Iterable[Record]:
        cursor = self._conn.execute(sql, parameters)
